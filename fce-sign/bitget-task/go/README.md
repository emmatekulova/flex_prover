# TEE Extension — Binance & Bitget Attestation (Go)

A TEE extension that fetches live Binance/Bitget account and market data inside a Trusted Execution Environment, signs it with the TEE's key, and publishes the attestation on-chain.
...
On-chain caller
  → InstructionSender.fetchBinanceUserProfileAndAttest()
    → TEE receives instruction
      → fetches account data from Binance/Bitget API (inside TEE)
...
| `BITGET_ACCOUNT_SUMMARY` | Spot balances + estimated USD | `/api/v2/spot/account/assets` | Yes |
