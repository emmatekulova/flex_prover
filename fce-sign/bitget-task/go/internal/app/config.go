...
const (
	...
	OpCommandBinanceFuturesDetails  = "BINANCE_FUTURES_DETAILS"

	OpCommandBitgetAccountSummary = "BITGET_ACCOUNT_SUMMARY"
)
...
// BitgetAPIKey returns the Bitget API key from environment, if set.
func BitgetAPIKey() string {
	return os.Getenv("BITGET_API_KEY")
}

// BitgetSecretKey returns the Bitget secret key from environment, if set.
func BitgetSecretKey() string {
	return os.Getenv("BITGET_SECRET_KEY")
}

// BitgetPassphrase returns the Bitget API passphrase from environment, if set.
func BitgetPassphrase() string {
	return os.Getenv("BITGET_PASSPHRASE")
}

// BitgetAPIBaseURL returns Bitget API base URL, defaulting to production.
