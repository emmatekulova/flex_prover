...
func main() {
	...
	source := flag.String("source", "binance", "data source: binance or bitget")
	flag.Parse()

	opCommand := "BINANCE_USER_PROFILE"
	if strings.ToLower(*source) == "bitget" {
		opCommand = "BITGET_ACCOUNT_SUMMARY"
	}

	// ── Step 1: fetch attestation from local extension ──────────────────────
	logger.Infof("Fetching %s attestation from extension...", *source)
	payload, signature, err := fetchAttestationFromExtension(*extURL, opCommand)
	if err != nil {
...
func fetchAttestationFromExtension(endpoint string, opCommand string) ([]byte, []byte, error) {
	reqBytes := []byte("{}")
