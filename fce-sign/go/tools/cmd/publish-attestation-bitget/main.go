// publish-attestation-bitget fetches a Bitget user profile attestation from the local extension,
...
// Usage:
//
//	go run ./cmd/publish-attestation-bitget \
...
func main() {
	...
	days := flag.Int("days", 7, "analysis window in days")
	flag.Parse()
	...
	logger.Infof("Fetching Bitget profile attestation from extension (window=%d days)...", *days)
...
func fetchAttestationFromExtension(endpoint, apiKey, secretKey, passphrase, wallet string, days int) ([]byte, []byte, error) {
	...
	df := dataFixed{
		...
		OpCommand:       stringToBytes32Hex("BITGET_PROFILE_GROWTH"),
	...
	body := actionRequest{Data: actionData{
		...
		Type:          "instruction",
		SubmissionTag: "publish-attestation-bitget",
