// publish-attestation-individual-trades-bitget fetches a Bitget individual-trades
// attestation from the local TEE extension, then calls
// BinanceAttestationStore.publishAttestation() on-chain so the TEE-signed
// payload becomes a permanent, verifiable on-chain record.
//
// Usage:
//
//	go run ./cmd/publish-attestation-individual-trades-bitget \
...
func main() {
	...
	storeAddr := flag.String("store", defaultStore, "deployed BinanceAttestationStore address (deploy new if empty)")
	artifactPath := flag.String("artifact", "../../contract/out/BinanceAttestationStore.sol/BinanceAttestationStore.json", "forge artifact path")
	...
	assets := flag.String("assets", "", "comma-separated list of assets to attest (empty = all)")
	flag.Parse()

	if strings.TrimSpace(*apiKey) == "" || strings.TrimSpace(*secretKey) == "" || strings.TrimSpace(*passphrase) == "" {
		logger.Fatalf("-apiKey, -secretKey, and -passphrase are required")
	}
	...
	if strings.TrimSpace(*assets) != "" {
		for _, a := range strings.Split(*assets, ",") {
			if t := strings.TrimSpace(a); t != "" {
				selectedAssets = append(selectedAssets, strings.ToUpper(t))
			}
		}
	}

	logger.Infof("Fetching Bitget individual-trades attestation from extension (assets=%v)...", selectedAssets)
	payload, signature, err := fetchAttestationFromExtension(
		*extURL,
...
func fetchAttestationFromExtension(endpoint, apiKey, secretKey, passphrase, wallet string, selectedAssets []string) ([]byte, []byte, error) {
	...
	df := dataFixed{
		...
		OpCommand:       stringToBytes32Hex("BITGET_INDIVIDUAL_TRADES"),
	...
	body := actionRequest{Data: actionData{
		...
		Type:          "instruction",
		SubmissionTag: "publish-attestation-individual-trades-bitget",
