// publish-attestation-individual-trades fetches a Binance individual-trades
...
// Usage:
//
//	go run ./cmd/publish-attestation-individual-trades \
...
func main() {
	...
	logger.Infof("Fetching Binance individual-trades attestation from extension (assets=%v)...", selectedAssets)
...
func fetchAttestationFromExtension(endpoint, apiKey, secretKey, wallet string, selectedAssets []string) ([]byte, []byte, error) {
	...
	df := dataFixed{
		...
		OpCommand:       stringToBytes32Hex("BINANCE_INDIVIDUAL_TRADES"),
	...
	body := actionRequest{Data: actionData{
		...
		Type:          "instruction",
		SubmissionTag: "publish-attestation-individual-trades",
